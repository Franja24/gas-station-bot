import importlib
import sys
import types
import unittest
from unittest.mock import patch


case_runner_stub = types.ModuleType("case_runner")
case_runner_stub.run_stages = lambda stages: {"stages": []}

login_if_needed_stub = types.ModuleType("features.login_if_needed")
login_if_needed_stub.run = lambda: None

payment_screen_close_app_stub = types.ModuleType(
    "features.payment_screen_close_app"
)
payment_screen_close_app_stub.run = lambda: None

start_kiosk_session_stub = types.ModuleType("features.start_kiosk_session")
start_kiosk_session_stub.run = lambda: None

_STUBBED_MODULES = {
    "case_runner": case_runner_stub,
    "features.login_if_needed": login_if_needed_stub,
    "features.payment_screen_close_app": payment_screen_close_app_stub,
    "features.start_kiosk_session": start_kiosk_session_stub,
}

_original_modules = {
    module_name: sys.modules.get(module_name)
    for module_name in _STUBBED_MODULES
}

sys.modules.update(_STUBBED_MODULES)
payment_screen_close_e2e = importlib.import_module(
    "features.payment_screen_close_e2e"
)

for module_name, original_module in _original_modules.items():
    if original_module is None:
        sys.modules.pop(module_name, None)
    else:
        sys.modules[module_name] = original_module

features_package = sys.modules.get("features")

if features_package is not None:
    for feature_name in (
        "login_if_needed",
        "payment_screen_close_app",
        "start_kiosk_session",
    ):
        feature_module = getattr(features_package, feature_name, None)
        if feature_module in _STUBBED_MODULES.values():
            delattr(features_package, feature_name)


class PaymentScreenCloseE2EFlowTests(unittest.TestCase):
    @patch("features.payment_screen_close_e2e.run_stages")
    def test_runs_without_open_kiosco_when_kiosk_is_already_open(
        self,
        run_stages_mock,
    ):
        expected_result = {"stages": []}
        run_stages_mock.return_value = expected_result

        result = payment_screen_close_e2e.run()

        self.assertIs(result, expected_result)
        stages = run_stages_mock.call_args.args[0]
        self.assertEqual(
            [stage_name for stage_name, _stage_function in stages],
            [
                "00_start_kiosk_session",
                "01_login_if_needed",
                "02_payment_screen_close_app",
            ],
        )
        self.assertEqual(
            [stage_function for _stage_name, stage_function in stages],
            [
                payment_screen_close_e2e.start_kiosk_session_run,
                payment_screen_close_e2e.login_if_needed_run,
                payment_screen_close_e2e.payment_screen_close_app_run,
            ],
        )
        self.assertFalse(
            hasattr(payment_screen_close_e2e, "open_kiosco_run")
        )


if __name__ == "__main__":
    unittest.main()
