...
premium_stub.run = lambda: None

cancel_stub = types.ModuleType("features.cancel")
cancel_stub.run = lambda: None
...
cancel_e2e = importlib.import_module("features.cancel_e2e")
...
class CancelE2EFlowTests(unittest.TestCase):
    @patch("features.cancel_e2e.run_stages")
    def test_runs_cancel_e2e_flow_as_reportable_stages(self, run_stages_mock):
        expected_result = {"stages": []}
        run_stages_mock.return_value = expected_result

        result = cancel_e2e.run()
        ...
        self.assertEqual(
            ...
            [
                cancel_e2e.open_kiosco_run,
                cancel_e2e.login_run,
                cancel_e2e.premium_run,
                cancel_e2e.cancel_run,
            ],
        )
