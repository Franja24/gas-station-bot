...
from config.settings import REFERENCE_SCREEN_SIZE, SCREENSHOT_TO_MOUSE_SCALE


class ClickImageTests(unittest.TestCase):
    @patch("clicker.click_coordinates", return_value=True)
    @patch("clicker.find_image")
    def test_calibrated_target_prefers_image_detection(
        ...
        click_coordinates_mock.assert_called_once_with(
            int(1000 * SCREENSHOT_TO_MOUSE_SCALE),
            int(600 * SCREENSHOT_TO_MOUSE_SCALE),
        )

    @patch("clicker.click_coordinates", return_value=True)
    @patch("clicker.find_image")
    def test_magna_falls_back_to_calibrated_coordinates_when_missing(
        self, find_image_mock, click_coordinates_mock
    ):
    ...
    @patch("clicker.click_coordinates", return_value=True)
    @patch("clicker.find_image")
    def test_sevenly_prefers_detected_asset(
        self, find_image_mock, click_coordinates_mock
    ):
        find_image_mock.return_value = SimpleNamespace(x=1434, y=244)
        ...
        find_image_mock.assert_called_once()
        click_coordinates_mock.assert_called_once_with(
            int(1434 * SCREENSHOT_TO_MOUSE_SCALE),
            int(244 * SCREENSHOT_TO_MOUSE_SCALE),
        )

    @patch("clicker.click_coordinates", return_value=True)
    @patch("clicker.find_image")
    def test_telefon_number_prefers_detected_asset(
        self, find_image_mock, click_coordinates_mock
    ):
        find_image_mock.return_value = SimpleNamespace(x=1400, y=770)
        ...
        find_image_mock.assert_called_once()
        click_coordinates_mock.assert_called_once_with(
            int(1400 * SCREENSHOT_TO_MOUSE_SCALE),
            int(770 * SCREENSHOT_TO_MOUSE_SCALE),
        )
